...
@interface NWStatusBarStyle (Hidden)
+ (NSArray*)allDefaultStyleIdentifier;
+ (NWStatusBarStyle*)defaultStyleWithName:(NSString*)styleName;
@end

@interface NWStatusBarNotificationViewController : UIViewController
@end

@interface NWStatusBarNotification ()
@property (nonatomic, strong, readonly) UIWindow *overlayWindow;
@property (nonatomic, strong, readonly) UIView *progressView;
@property (nonatomic, strong, readonly) NWStatusBarView *topBar;

@property (nonatomic, strong) NSTimer *dismissTimer;
@property (nonatomic, assign) CGFloat progress;

@property (nonatomic, weak) NWStatusBarStyle *activeStyle;
@property (nonatomic, strong) NWStatusBarStyle *defaultStyle;
@property (nonatomic, strong) NSMutableDictionary *userStyles;
@end


@implementation NWStatusBarNotification
...
+ (NWStatusBarNotification*)sharedInstance {
    static dispatch_once_t once;
    static NWStatusBarNotification *sharedInstance;
    dispatch_once(&once, ^ {
        sharedInstance = [[NWStatusBarNotification alloc] init];
    });
...
{
    [[NWStatusBarNotification sharedInstance] dismissAnimated:animated];
}

+ (void)dismissAfter:(NSTimeInterval)delay;
{
    [[NWStatusBarNotification sharedInstance] setDismissTimerWithInterval:delay];
}

+ (void)setDefaultStyle:(NWPrepareStyleBlock)prepareBlock;
{
    NSAssert(prepareBlock != nil, @"No prepareBlock provided");
    
    NWStatusBarStyle *style = [[self sharedInstance].defaultStyle copy];
    [NWStatusBarNotification sharedInstance].defaultStyle = prepareBlock(style);
}

+ (NSString*)addStyleNamed:(NSString*)identifier
                   prepare:(NWPrepareStyleBlock)prepareBlock;
{
    return [[NWStatusBarNotification sharedInstance] addStyleNamed:identifier
                                                           prepare:prepareBlock];
}

+ (void)showProgress:(CGFloat)progress;
{
    [[NWStatusBarNotification sharedInstance] setProgress:progress];
}

+ (void)showActivityIndicator:(BOOL)show indicatorStyle:(UIActivityIndicatorViewStyle)style;
{
    [[NWStatusBarNotification sharedInstance] showActivityIndicator:show indicatorStyle:style];
}

+ (BOOL)isVisible;
{
    return [[NWStatusBarNotification sharedInstance] isVisible];
}
...
{
    self.defaultStyle = [NWStatusBarStyle defaultStyleWithName:NWStatusBarStyleDefault];
    
    self.userStyles = [NSMutableDictionary dictionary];
    for (NSString *styleName in [NWStatusBarStyle allDefaultStyleIdentifier]) {
        [self.userStyles setObject:[NWStatusBarStyle defaultStyleWithName:styleName] forKey:styleName];
    }
}
                   ...
                   prepare:(NWPrepareStyleBlock)prepareBlock;
{
    ...
    NWStatusBarStyle *style = [self.defaultStyle copy];
...
- (UIView*)showWithStatus:(NSString *)status
                    style:(NWStatusBarStyle*)style;
{
    // prepare for new style
    if (style != self.activeStyle) {
        self.activeStyle = style;
        if (self.activeStyle.animationType == NWStatusBarAnimationTypeFade) {
    ...
    // animate in
    BOOL animationsEnabled = (style.animationType != NWStatusBarAnimationTypeNone);
    if (animationsEnabled && style.animationType == NWStatusBarAnimationTypeBounce) {
...
- (void)dismissAnimated:(BOOL)animated;
{
    ...
    BOOL animationsEnabled = (self.activeStyle.animationType != NWStatusBarAnimationTypeNone);
...
- (void)animateInWithBounceAnimation;
{
    ...
    [self.topBar.layer addAnimation:animation forKey:@"NWBounceAnimation"];
}
...
- (void)setProgress:(CGFloat)progress;
{
    ...
    // update superview
    if (self.activeStyle.progressBarPosition == NWStatusBarProgressBarPositionBelow ||
        self.activeStyle.progressBarPosition == NWStatusBarProgressBarPositionNavBar) {
    ...
    if (self.activeStyle.progressBarPosition == NWStatusBarProgressBarPositionBottom) {
        frame.origin.y = barHeight - height;
    } else if(self.activeStyle.progressBarPosition == NWStatusBarProgressBarPositionCenter) {
        frame.origin.y = round((barHeight - height)/2.0);
    } else if(self.activeStyle.progressBarPosition == NWStatusBarProgressBarPositionTop) {
        frame.origin.y = 0.0;
    } else if(self.activeStyle.progressBarPosition == NWStatusBarProgressBarPositionBelow) {
        frame.origin.y = barHeight;
    } else if(self.activeStyle.progressBarPosition == NWStatusBarProgressBarPositionNavBar) {
...
- (UIWindow *)overlayWindow;
{
    if(_overlayWindow == nil) {
        ...
        _overlayWindow.rootViewController = [[NWStatusBarNotificationViewController alloc] init];
...
{
    if(_topBar == nil) {
        _topBar = [[NWStatusBarView alloc] init];
        [self.overlayWindow.rootViewController.view addSubview:_topBar];
        
        NWStatusBarStyle *style = self.activeStyle ?: self.defaultStyle;
        if (style.animationType != NWStatusBarAnimationTypeFade) {
            self.topBar.transform = CGAffineTransformMakeTranslation(0, -self.topBar.frame.size.height);
        } else {
            self.topBar.alpha = 0.0;
        }
...
@implementation NWStatusBarNotificationViewController
